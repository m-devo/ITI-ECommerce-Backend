...
const getBooks = catchAsync(async (req, res, next) => {
  ...
  const books = await Book.find({}, { "__v": 0, "_id": 0 })
  ...
  if (!books.length) {
    return res
      .status(404)
      .json(new ApiResponse(404, [], 'No books found'));
  }
...
const getOneBook = catchAsync(async (req, res, next) => {
  ...
  const book = await Book.findById(ID, { "__v": 0, "_id": 0 });
...
const deleteBook = catchAsync(async (req, res, next) => {
  ...
  if (book.bookPath && fs.existsSync(book.bookPath)) {
    try {
      fs.unlinkSync(book.bookPath);
      console.log(`Deleted book file: ${book.bookPath}`);
    } catch (err) {
      console.error(`Failed to delete book file: ${book.bookPath}`, err);
    }
  }
  ...
  if (book.imagePath && fs.existsSync(book.imagePath)) {
    try {
      fs.unlinkSync(book.imagePath);
      console.log(`Deleted image file: ${book.imagePath}`);
    } catch (err) {
      console.error(`Failed to delete image file: ${book.imagePath}`, err);
    }
  }
